...
<section class="bg-white">
    <nav class="py-10 mx-16 flex items-center justify-between">
        ...
        <a class="text-black font-medium ml-auto hover:bg-biru_hover hover:text-white py-3 px-4 rounded-full duration-300 {{ Request::path() == 'admin/dokumen' ? 'bg-biru_hover text-white' : '' }}" href="{{route('dashboard')}}" >Dashboard</a>
    ...
    <div class="flex items-center">
        <div class="ml-20">
            <p class="text-4xl font-bold">Karir</p>
            <p class="mt-3">Tabel data seluruh karir ada di halaman ini.</p>
            
        </div>
        <div class="ml-auto mr-20">
            <a href="{{route('karir.tambah')}}" class=" ml-20 py-3 px-4 bg-biru-tua hover:bg-biru_hover text-white rounded-full">Tambah</a>
        </div>
    </div>
                    ...
                    <table class="min-w-full leading-normal">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">{{ $doc->nama_lowongan }}</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        {{ $doc->tanggal_upload }}
                                ...
                                <td class="py-5 border-b border-gray-200 bg-white text-sm">
                                    <div
                                        ...
                                        <a href="{{ route('karir.edit', $doc->id) }}" class="relative">Edit</a>
                                    </div>
                                    <div
...
<div id="deleteModal{{ $doc->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        ...
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <p class="mb-4 text-gray-500 dark:text-gray-300">Apakah anda yakin ingin menghapus data karir ini?</p>
            <div class="flex justify-center items-center space-x-4">
                <button type="button" data-modal-toggle="deleteModal{{ $doc->id }}" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600" onclick="closeModal('{{ $doc->id }}')">
                    Tidak
                </button>
                <form action="{{ route('karir.destroy', $doc->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
